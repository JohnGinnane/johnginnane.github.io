...
for game in games:
    ...
    for s in sets:
        set_index += 1
        game_dict[gamenum][str(set_index)] = {}
        ...
        for m in match:
            ...
            if count:
                ...
                if colour in game_dict[gamenum][str(set_index)]:
                    if colour_count > game_dict[gamenum][str(set_index)][colour]:
                        game_dict[gamenum][str(set_index)][colour] = colour_count
                else:
                    game_dict[gamenum][str(set_index)][colour] = colour_count
